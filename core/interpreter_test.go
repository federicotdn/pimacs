...
func TestReadEvalPrint(t *testing.T) {
	...
	testStringToString(t, readEvalPrin1, []stringToStringTC{
		...
		{"(plist-put nil :foo 1)", "(:foo 1)", nil},
		{"(plist-put (cons 1 2) :a 1)", "", anyError},
		{"(plist-put (1 2 3 . 4) :a 1)", "", anyError},
		{"(plist-put 99 :a 1)", "", anyError},
		...
		{"(progn (fset 'foo (function (lambda (&optional x y) x))) (foo))", "nil", nil},
		...
		{"(setq a 1)", "1", nil},
		{"(setq a 1 b 2)", "2", nil},
		{"(setq a 1 b 2 c)", "", anyError},
		{"(progn (setq a 1) 1)", "1", nil},
		{"(let)", "", anyError},
		{"(let (a) a)", "nil", nil},
		{"(let ((a nil)) a)", "nil", nil},
