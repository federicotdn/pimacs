...
- A file called `globals.h` with the actual declaration of all symbols, variables and functions that have been declared throughout the codebase. This file allows, for example, C code to use `Qfoo` to refer to the symbol `foo`. `Qfoo` will be a macro that returns a `Lisp_Object` representing the symbol. Note: this is not related to the `obarray`.
- A file called `etc/DOC`. See next section.
- Maybe more things, I am not sure.

Since Go does not have macros, and I want to avoid source code generation as much as possible (or completely), so Pimacs takes a slightly different approach. I attempted to make the process of declaring new functions, symbols and variables as least annoying as possible.

First, all statically defined symbols must be defined in the `lispGlobals` structure in `symbols.go`. "Declaring a function" and "declaring a variable" just mean declaring a symbol and then assigning it a function value or data value, respectively (Elisp is a "Lisp-2"). The symbol must then be assigned a name via the `createSymbols` method function (the name visible from Elisp, that is). Lastly, in case of functions or variables, a call to `defSubrX` or `defVar` will be necessary. Errors can be declared as well in a similar fashion, using `putError` instead.
...
In order to declare a function:
1. Add it to `lispGlobals`.
2. Initialize it in `createSymbols`.
